...
export function ArchitecturePropertiesPanel({ object, onChange }: ArchitecturePropertiesPanelProps) {
    ...
    const renderDoorForm = () => (
        <div className="space-y-6">
            ...
            {/* Swing Direction - For all doors */}
            <div className="space-y-2">
                <label className="text-sm font-medium text-foreground flex items-center gap-2">
                    <Copy size={16} className="text-primary" /> Dirección de Apertura
                </label>
                <div className="flex gap-1 bg-input p-1 rounded-md border border-border">
                    <button
                        onClick={() => onChange({ swingDirection: 'left' })}
                        className={cn(
                            "flex-1 py-1.5 rounded-sm text-xs font-medium transition-colors",
                            (!object.swingDirection || object.swingDirection === 'left') ? "bg-primary text-primary-foreground shadow-sm" : "hover:bg-accent text-muted-foreground"
                        )}
                    >
                        {object.doorType === 'double' ? '↙ Adentro' : '← Izquierda'}
                    </button>
                    <button
                        onClick={() => onChange({ swingDirection: 'right' })}
                        className={cn(
                            "flex-1 py-1.5 rounded-sm text-xs font-medium transition-colors",
                            object.swingDirection === 'right' ? "bg-primary text-primary-foreground shadow-sm" : "hover:bg-accent text-muted-foreground"
                        )}
                    >
                        {object.doorType === 'double' ? 'Afuera ↗' : 'Derecha →'}
                    </button>
                </div>
                <p className="text-xs text-muted-foreground">
                    {object.doorType === 'double' ? 'Si las hojas abren hacia adentro o afuera' : 'Hacia qué lado abre la puerta'}
                </p>
            </div>
