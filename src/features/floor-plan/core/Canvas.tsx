...
export interface FloorObject {
    ...
    // For merged tables
    mergedGroup?: string; // UUID - tables with same mergedGroup are joined
}
...
export const Canvas = ({
    ...
    // Calculate total covers across all tables
    const totalCovers = useMemo(() => {
        let total = 0;
        objects.filter(o => o.type === 'table' || o.type === 'bar').forEach(obj => {
            const getChairCount = (sideLength: number) => Math.max(1, Math.floor(sideLength / chairSpacingCm));
            if (obj.seating?.top?.enabled) total += getChairCount(obj.width);
            if (obj.seating?.bottom?.enabled) total += getChairCount(obj.width);
            if (obj.seating?.left?.enabled) total += getChairCount(obj.height);
            if (obj.seating?.right?.enabled) total += getChairCount(obj.height);
        });
        return total;
    }, [objects, chairSpacingCm]);
    ...
    const renderDoor = (obj: FloorObject, key: string, props: any) => {
        ...
        const renderSingleDoor = () => {
            // swingDirection: 'left' = hinge on left side (default), 'right' = hinge on right side (mirrored)
            const isFlipped = obj.swingDirection === 'right';

            if (isFlipped) {
                // Hinge at top-right (x + width, y)
                // Panel extends down (y + doorWidth)
                // Arc goes from Tip (x + width, y + doorWidth) to Latch (x, y)
                return (
                    <g>
                        {/* Swing Arc */}
                        <path
                            d={`M ${obj.x + obj.width} ${obj.y + doorWidth} Q ${obj.x} ${obj.y + doorWidth} ${obj.x} ${obj.y}`}
        ...
        const renderDoubleDoor = () => {
            ...
            // swingDirection: 'left' = opens inward (default), 'right' = opens outward
            const opensOutward = obj.swingDirection === 'right';

            if (opensOutward) {
                // Arcs open upward (away from y + doorWidth)
                return (
                    <g>
                        {/* Left Arc - curves up */}
                        <path d={`M ${obj.x} ${obj.y} Q ${midX} ${obj.y} ${midX} ${obj.y + doorWidth}`} fill="none" stroke={strokeColor} strokeWidth={1} strokeDasharray="4 2" vectorEffect="non-scaling-stroke" />
                        {/* Right Arc - curves up */}
                        <path d={`M ${obj.x + obj.width} ${obj.y} Q ${midX} ${obj.y} ${midX} ${obj.y + doorWidth}`} fill="none" stroke={strokeColor} strokeWidth={1} strokeDasharray="4 2" vectorEffect="non-scaling-stroke" />
                        {/* Left Panel */}
                        <rect x={obj.x} y={obj.y + doorWidth - panelThick} width={panelThick} height={panelThick} fill={panelFill} stroke={strokeColor} strokeWidth={2} />
                        {/* Right Panel */}
                        <rect x={obj.x + obj.width - panelThick} y={obj.y + doorWidth - panelThick} width={panelThick} height={panelThick} fill={panelFill} stroke={strokeColor} strokeWidth={2} />
                    </g>
                );
            }

            // Default: opens inward (arcs go down from top)
            return (
                <g>
                    {/* Left Arc */}
    ...
    const handleObjectMouseDown = (e: React.MouseEvent, id: string, objX: number, objY: number) => {
        ...
        // Check if clicked object is part of a merged group
        const clickedObj = objects.find(o => o.id === id);
        const mergedGroupId = clickedObj?.mergedGroup;

        // Get all table IDs in the same merged group
        const groupTableIds = mergedGroupId
            ? objects.filter(o => o.mergedGroup === mergedGroupId).map(o => o.id)
            : [id];

        if (e.shiftKey) {
            if (newSelectedIds.has(id)) {
                // Remove entire group
                groupTableIds.forEach(tid => newSelectedIds.delete(tid));
            } else {
                // Add entire group
                groupTableIds.forEach(tid => newSelectedIds.add(tid));
            }
        } else {
            // If clicking an object already in selection (and no Shift), KEEP selection (for dragging group)
            // If clicking unselected object, clear and select only this/group
            if (!newSelectedIds.has(id)) {
    ...
    // === DIMENSION GUIDES (ArchiCAD-style) ===
    // Calculate distances to nearby objects when dragging
    const dimensionGuides = useMemo(() => {
        ...
        // Calculate edge positions of dragged object
        const dragged = {
            left: draggedObj.x,
            right: draggedObj.x + draggedObj.width,
            top: draggedObj.y,
            bottom: draggedObj.y + draggedObj.height,
            centerX: draggedObj.x + draggedObj.width / 2,
            centerY: draggedObj.y + draggedObj.height / 2,
        };
        ...
        // Check all other objects (excluding walls, doors, windows)
        const otherObjects = objects.filter(o =>
            o.id !== draggedId &&
            (o.type === 'table' || o.type === 'bar')
        );
        ...
        for (const other of otherObjects) {
            ...
            // Check horizontal alignment (same Y range approximately)
            const yOverlap = !(dragged.bottom < otherEdges.top || dragged.top > otherEdges.bottom);
            // Check vertical alignment  
            const xOverlap = !(dragged.right < otherEdges.left || dragged.left > otherEdges.right);

            if (yOverlap) {
                // Horizontal distance (object is to the left or right)
                if (otherEdges.right <= dragged.left) {
                    // Other is to the LEFT
                    const distance = dragged.left - otherEdges.right;
                    if (distance > 0 && distance < 300) {
                        const midY = (Math.max(dragged.top, otherEdges.top) + Math.min(dragged.bottom, otherEdges.bottom)) / 2;
                        guides.push({
                            distance,
                            direction: 'horizontal',
                            x1: otherEdges.right, y1: midY,
                            x2: dragged.left, y2: midY,
                            labelX: (otherEdges.right + dragged.left) / 2,
                            labelY: midY - 12,
                        });
                    }
                } else if (otherEdges.left >= dragged.right) {
                    // Other is to the RIGHT
                    const distance = otherEdges.left - dragged.right;
                    if (distance > 0 && distance < 300) {
                        const midY = (Math.max(dragged.top, otherEdges.top) + Math.min(dragged.bottom, otherEdges.bottom)) / 2;
                        guides.push({
                            distance,
                            direction: 'horizontal',
                            x1: dragged.right, y1: midY,
                            x2: otherEdges.left, y2: midY,
                            labelX: (dragged.right + otherEdges.left) / 2,
                            labelY: midY - 12,
                        });
                    }
                }
            ...
            if (xOverlap) {
                // Vertical distance (object is above or below)
                if (otherEdges.bottom <= dragged.top) {
                    // Other is ABOVE
                    const distance = dragged.top - otherEdges.bottom;
                    if (distance > 0 && distance < 300) {
                        const midX = (Math.max(dragged.left, otherEdges.left) + Math.min(dragged.right, otherEdges.right)) / 2;
                        guides.push({
                            distance,
                            direction: 'vertical',
                            x1: midX, y1: otherEdges.bottom,
                            x2: midX, y2: dragged.top,
                            labelX: midX + 8,
                            labelY: (otherEdges.bottom + dragged.top) / 2,
                        });
                    }
                } else if (otherEdges.top >= dragged.bottom) {
                    // Other is BELOW
                    const distance = otherEdges.top - dragged.bottom;
                    if (distance > 0 && distance < 300) {
                        const midX = (Math.max(dragged.left, otherEdges.left) + Math.min(dragged.right, otherEdges.right)) / 2;
                        guides.push({
                            distance,
                            direction: 'vertical',
                            x1: midX, y1: dragged.bottom,
                            x2: midX, y2: otherEdges.top,
                            labelX: midX + 8,
                            labelY: (dragged.bottom + otherEdges.top) / 2,
                        });
                    }
                }
            }
        ...
        // Sort by distance and return only closest 4
        return guides.sort((a, b) => a.distance - b.distance).slice(0, 4);
    }, [draggedId, objects]);

    // Render function for dimension guides  
    const renderDimensionGuides = () => {
        if (dimensionGuides.length === 0) return null;

        return (
            <g className="dimension-guides" style={{ pointerEvents: 'none' }}>
                {dimensionGuides.map((guide, idx) => (
                    <g key={idx}>
                        ...
                        {guide.direction === 'horizontal' ? (
                            <>
                                <line x1={guide.x1} y1={guide.y1 - 6} x2={guide.x1} y2={guide.y1 + 6} stroke="#3b82f6" strokeWidth={1.5} vectorEffect="non-scaling-stroke" />
                                <line x1={guide.x2} y1={guide.y2 - 6} x2={guide.x2} y2={guide.y2 + 6} stroke="#3b82f6" strokeWidth={1.5} vectorEffect="non-scaling-stroke" />
                            </>
                        ) : (
                            <>
                                <line x1={guide.x1 - 6} y1={guide.y1} x2={guide.x1 + 6} y2={guide.y1} stroke="#3b82f6" strokeWidth={1.5} vectorEffect="non-scaling-stroke" />
                                <line x1={guide.x2 - 6} y1={guide.y2} x2={guide.x2 + 6} y2={guide.y2} stroke="#3b82f6" strokeWidth={1.5} vectorEffect="non-scaling-stroke" />
                            </>
                        )}
        ...
        <div className="relative h-full w-full overflow-hidden bg-transparent select-none">
            ...
            >
                ...
                >
                        ...
                        {/* Objects */}
                        {
                            objects.map(obj => {
                                ...
                                if (obj.type === "table" || obj.type === "bar") {
                                    ...
                                    // Check if this table is available for merge (in merge mode, not already in a group, not the source table)
                                    const isMergeAvailable = mergeMode && obj.type === 'table' && !obj.mergedGroup && !tablesToMerge.has(obj.id);
                                    const isMergeSource = tablesToMerge.has(obj.id);

                                    // For merged tables, we'll draw a group border separately - don't show individual selection border
                                    const isMergedTable = !!obj.mergedGroup;
                                        ...
                                        <g key={obj.id} transform={`rotate(${obj.rotation}, ${centerX}, ${centerY})`}>
                                            ...
                                            {isCircular ? (
                                                <ellipse
                                                    ...
                                                    rx={obj.width / 2}
                                                    ry={obj.height / 2}
                                                    fill={isMergeAvailable ? "#dcfce7" : isMergeSource ? "#bbf7d0" : obj.status === "occupied" ? "#fee2e2" : (isBar ? "#fefce8" : "#fafafa")}
                                                    stroke={isMergeAvailable ? "#22c55e" : isMergeSource ? "#16a34a" : (isSelected && !isMergedTable) ? undefined : (obj.status === "occupied" ? "#ef4444" : "#a8a29e")}
                                                    strokeWidth={isMergeAvailable || isMergeSource ? 3 : (isSelected && !isMergedTable) ? 3 : (obj.status === "occupied" ? 2 : 1.5)}
                                                    className={cn(
                                                        "transition-colors",
                                                        isInteracting ? "cursor-move hover:stroke-primary" : "pointer-events-none",
                                                        (isSelected && !isMergedTable) && "stroke-primary",
                                                        isMergeAvailable && "animate-pulse cursor-pointer"
                                                    )}
                                                    onMouseDown={(e) => handleObjectMouseDown(e, obj.id, obj.x, obj.y)}
                                                    style={isMergeAvailable ? { pointerEvents: 'auto' } : undefined}
                                                />
                                            ) : (
                                                <rect
                                                    x={obj.x}
                                            ...
                                            {/* Label - for merged tables, skip (rendered separately after all objects) */}
                                            {obj.label && (() => {
                                                // Skip merged tables - their labels are rendered after all objects
                                                if (obj.mergedGroup) {
                                                    return null;
                                                }

                                                // Normal table - render label with pill style
                                                const labelText = obj.status === "occupied" && obj.customerName ? obj.customerName : obj.label;
                                                const fontSize = Math.min(obj.width, obj.height) * 0.22;
                                                const bgWidth = Math.max(fontSize * 2.5, obj.width * 0.6);
                                                const bgHeight = fontSize * 1.6;

                                                return (
                                                    <g transform={`rotate(${-obj.rotation}, ${centerX}, ${centerY})`} className="pointer-events-none">
                                                        <rect
                                                            x={centerX - bgWidth / 2}
                    ...
                    {/* Merged Table Labels - rendered AFTER all objects for proper z-index */}
                    {(() => {
                        // Group tables by mergedGroup
                        const mergedGroups = new Map<string, FloorObject[]>();
                        objects.forEach(obj => {
                            if (obj.mergedGroup && obj.type === 'table') {
                                const group = mergedGroups.get(obj.mergedGroup) || [];
                                group.push(obj);
                                mergedGroups.set(obj.mergedGroup, group);
                            }
                        });

                        return Array.from(mergedGroups.entries()).map(([groupId, groupTables]) => {
                            // Find the table with the combined label
                            const primaryTable = groupTables.find(t => t.label?.includes('+'));
                            ...
                            const labelText = primaryTable.status === "occupied" && primaryTable.customerName
                                ? primaryTable.customerName
                                : primaryTable.label || '';
                            const fontSize = Math.min(groupWidth, groupHeight) * 0.25;

                            const bgWidth = Math.max(fontSize * 3, groupWidth * 0.5);
                            const bgHeight = fontSize * 1.6;

                            // Check if this group is selected
                            const isGroupSelected = groupTables.some(t =>
                                selectedIds?.has(t.id) || selectedId === t.id
                            );

                            return (
                                <g key={`merged-label-${groupId}`} className="pointer-events-none">
                                    {/* Selection border around entire group */}
                                    {isGroupSelected && (
                                        <rect
                                            x={minX - 4}
                                            y={minY - 4}
                                    ...
                                    {/* Label pill */}
                                    <rect
                                        x={groupCenterX - bgWidth / 2}
                                        y={groupCenterY - bgHeight / 2}
                                        width={bgWidth}
                                        height={bgHeight}
                                        rx={bgHeight / 2}
                                        fill="white"
                ...
                <div className="absolute top-6 left-6 bg-background/90 backdrop-blur px-4 py-2.5 rounded-xl shadow-sm border flex items-center gap-3">
                    <div className="text-2xl">🍴</div>
                    <div className="flex flex-col">
                        <span className="text-2xl font-bold text-foreground leading-none">{totalCovers}</span>
