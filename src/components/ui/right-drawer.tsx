...
interface RightDrawerProps {
    ...
    onClose: () => void;
    title?: string;
    onTitleChange?: (newTitle: string) => void; // Makes title editable
    titlePrefix?: string; // e.g. "Mesa " - shown before editable part
    children: React.ReactNode;
    className?: string;
}

export function RightDrawer({ isOpen, onClose, title, onTitleChange, titlePrefix, children, className }: RightDrawerProps) {
            ...
            <div className="flex items-center justify-between p-4 border-b border-border">
                {onTitleChange ? (
                    <div className="flex items-center gap-2">
                        {titlePrefix && <span className="text-lg font-semibold text-foreground">{titlePrefix}</span>}
                        <input
                            type="text"
                            value={title || ''}
                            onChange={(e) => onTitleChange(e.target.value)}
                            className="text-lg font-semibold text-foreground bg-input border border-border rounded px-2 py-0.5 w-20 focus:border-primary focus:outline-none"
                            placeholder="ID..."
                        />
                    </div>
                ) : (
