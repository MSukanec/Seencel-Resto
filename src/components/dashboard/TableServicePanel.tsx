...
import { openTableSession, closeTableSession } from "@/lib/actions/table-actions";
                ...
                <div className="space-y-6 animate-in fade-in zoom-in-95 duration-300">
                    <div className="p-4 bg-muted/40 rounded-xl border border-border space-y-4">
                        <div className="flex items-center justify-between">
                            ...
                            <span className="px-2 py-1 rounded bg-red-500/10 text-red-600 text-xs font-bold border border-red-500/20 flex items-center gap-1">
                                <Clock size={12} /> Ocupada
                            </span>
                        </div>

                        <div className="space-y-1">
                            <span className="text-xs text-muted-foreground uppercase tracking-wider font-bold">Cliente</span>
                        ...
                        <div className="space-y-1">
                            <span className="text-xs text-muted-foreground uppercase tracking-wider font-bold">Comensales</span>
                            <div className="text-lg font-bold flex items-center gap-2">
                                <Users size={18} className="text-primary" />
                                {table.current_pax || pax} Personas
                            </div>
                        </div>

                        <div className="space-y-1">
                            <span className="text-xs text-muted-foreground uppercase tracking-wider font-bold">Abierto hace</span>
                            <div className="text-sm font-medium text-muted-foreground">
                                {table.opened_at ? new Date(table.opened_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : "Recién"}
                            </div>
                        </div>
                    </div>
                    ...
                    <button
                        onClick={async () => {
                            ...
                            setLoading(true);
                            const { error } = await closeTableSession(table.id);
                            if (!error) {
                                onUpdate();
                            } else {
                                alert("Error al cerrar mesa: " + error);
                            }
                            setLoading(false);
                        }}
                        disabled={loading}
                        className="w-full bg-destructive/10 text-destructive h-11 rounded-xl text-sm font-bold hover:bg-destructive/20 transition-all flex items-center justify-center gap-2"
                    >
                        {loading ? <Loader2 className="animate-spin" size={16} /> : "Cerrar Mesa"}
                    </button>
                </div>
            ) : (
                <>
